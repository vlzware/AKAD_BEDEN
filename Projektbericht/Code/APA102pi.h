...
// 11.2020: (vlzware) Simplified and updated to work with WiringPi on Raspberry Pi
...
#define _POLOLU_RGB_COLOR
  ...
  * color. */
    typedef struct rgb_color
    {
        ...
        rgb_color(){};
        rgb_color(std::uint8_t r, std::uint8_t g, std::uint8_t b) : red(r), green(g), blue(b){};
    } rgb_color;
#endif
  ...
  */
    ...
    */
        virtual void write(rgb_color *colors, std::uint16_t count, std::uint8_t brightness = 31) = 0;
    };
  ...
  */
    template <std::uint8_t dataPin, std::uint8_t clockPin>
    class APA102 : public APA102Base
    {
    public:
        virtual void write(rgb_color *colors, std::uint16_t count, std::uint8_t brightness = 31)
        {
            startFrame();
    ...
    * to control multiple LED strips. */
        void endFrame(std::uint16_t count)
        {
      ...
      * pixels if you try to update fewer LEDs than are on your LED strip. */
            ...
            for (std::uint16_t i = 0; i < (count + 14) / 16; i++)
            {
                transfer(0);
            }
    ...
    * documentation. */
        void sendColor(std::uint8_t red, std::uint8_t green, std::uint8_t blue, std::uint8_t brightness = 31)
        {
    ...
    * This is part of the low-level interface described in the startFrame()
    * documentation. */
        void sendColor(rgb_color color, std::uint8_t brightness = 31)
        {
